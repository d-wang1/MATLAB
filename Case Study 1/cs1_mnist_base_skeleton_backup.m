...
brokenIter = max_iter;
for iter=1:max_iter
    % Groups = the centroid each data point belongs to
    group = zeros(size(train,1),1);
    for j=1:size(train,1)
        minC = 1;
        dists = zeros([k,1]);
        for c=1:k
            dists(c) = norm(train(j,1:784)-centroids(c,1:784));
            cost_iteration(iter) = cost_iteration(iter) + dists(c);
    ...
    newLocs = splitapply(@mean,train(:,1:784),group);
    if(centroids(:,1:784)==newLocs)
        fprintf("Finished, iteration %d",iter)
        brokenIter = iter;
        break;
    end
    centroids(:,1:784) = newLocs;
end
...
cost_iteration
figure;
plot(cost_iteration(1:brokenIter),'o-')
title("K Means Cost Function")
xlabel("Iteration")
ylabel("Cost")
axis padded
%% This next section of code will make a plot of all of the centroids
% Again, use help <#functionname functionname> to learn about the different 
% functions that are being used here.
